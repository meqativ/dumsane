...
const onlyBuild = ["eval", "PluginRepo"]; // process.argv.slice(2, process.argv.length);
...
if (onlyBuild.length > 0)
  pluggers = pluggers.filter((p) => onlyBuild.includes(p));
...
for (let plug of pluggers) {
  ...
  try {
    ...
    console.log(
      prefix,
      `Successfully built ${
        manifest.name !== plug ? `"${manifest.name}"` : ``
      }@${plug}!\n`
    );
    built.succeeded++;
  } catch (e) {
